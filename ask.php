        ...
        <div class="row">
            ...
            <div class="span8 page-content">
                ...
                <form action="#" method="post" id="commentform">
                    ...
                    <div>
                        <label>Mô tả *</label>
                        <textarea id="Description" class="span8" required name="Description" cols="58" rows="10"
                            required></textarea>
                    </div>

                    <div>
                        <label>Nhập tag cho câu hỏi * (enter để thêm tag, backspace để xóa tag)</label>
                        <input type="text" data-role="tagsinput" required>
                    </div>
...
$(document).ready(function() {
    $.getJSON("api/category/read.php", function(data) {
        ...
        $.each(data.records, function(key, val) {
            categories_options_html += `<option value='` + val.ID + `'>` + val.catName +
                `</option>`;
        });
    ...
    $(document).on('submit', '#commentform', function() {
        ...
        if (jwt == "") {
            ...
            createQuestion.labelName = tagValue;
            ...
            $.ajax({
                ...
                success: function(result) {
                    $('#response').html(
                        "<div class='alert alert-success'>Đặt câu hỏi thành công.Vui lòng đợi Admin duyệt </div>"
                    );
                },
                error: function(xhr, resp, text) {
                    $('#response').html(
                        "<div class='alert alert-danger'>Đặt câu hỏi thất bại. Vui lòng thử lại.</div>"
                    );
                }
            });
        }
